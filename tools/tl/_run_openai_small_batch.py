"""Runner secuencial con batch chico para limpiar pendientes."""
...
print(f"[runner] {len(files)} archivos en {ROOT} (batch=10)", flush=True)

for i, f in enumerate(files, 1):
    print(f"\n[runner] === [{i}/{len(files)}] {f.name} ({f.stat().st_size//1024}KB) ===", flush=True)
    t0 = time.time()
    rc = subprocess.call([sys.executable, "-u", "tools/tl/translate.py", str(f),
                          "--provider", "openai", "--batch-size", "10"])
    dt = time.time() - t0
    if USAGE.exists():
